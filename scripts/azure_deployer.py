...
# Request bodies MUST be built from these model classes, never from plain dicts.
#
# azure-mgmt-network 31.x / azure-mgmt-compute 38.x are generated by the newer
# TypeSpec pipeline, whose operations serialize the body with
# `json.dumps(parameters, cls=SdkJSONEncoder)` — a dict is sent to ARM verbatim.
# The older SDK generation ran dicts through an attribute map that renamed keys,
# so snake_case dicts used to work. They no longer do: ARM rejected
# `{"security_rules": [...]}` with
#
#     (InvalidRequestContent) Could not find member 'security_rules' on object
#     of type 'ResourceDefinition'
#
# The models also nest their fields under `properties`, which no flat dict here
# ever did. Constructing them is the only version-portable option.
...
class AzureVMDeployer:
    ...
    def create_network_interface(self, name, subnet_id, nsg_id=None):
        ...
        try:
            ...
            nic_properties = NetworkInterfacePropertiesFormat(
                ip_configurations=[
                    NetworkInterfaceIPConfiguration(
            ...
            # Add NSG if provided
            if nsg_id:
                nic_properties.network_security_group = NetworkSecurityGroup(id=nsg_id)
        ...
        try:
            ...
            vm_parameters = VirtualMachine(
                ...
                properties=VirtualMachineProperties(
                    hardware_profile=HardwareProfile(vm_size=actual_vm_size),
                    storage_profile=StorageProfile(
                        image_reference=ImageReference(**settings.VM_IMAGE),
                        os_disk=OSDisk(
                    ...
                    network_profile=NetworkProfile(
                        network_interfaces=[
                            NetworkInterfaceReference(id=nic_id, delete_option='Delete')
                        ]
                    ),
                            ...
                            ssh=SshConfiguration(
                                public_keys=[
                                    SshPublicKey(
                                        ...
                                        key_data=settings.SSH_PUBLIC_KEY,
                                    )
                                ]
                            ),
                        ),
                    ...
                    security_profile=SecurityProfile(
                        uefi_settings=UefiSettings(
                            secure_boot_enabled=settings.ENABLE_SECURE_BOOT,
                            v_tpm_enabled=settings.ENABLE_VTPM,
                        ),
    ...
    '''
            ...
            extension_params = VirtualMachineExtension(
                ...
                properties=VirtualMachineExtensionProperties(
                    ...
                    settings={'script': encoded_script},
                    protected_settings={},
                ),
            )
